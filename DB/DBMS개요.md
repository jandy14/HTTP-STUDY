...
[Redis](../CACHE/Redis.md)를 공부하다 다시 샤딩에 대해 알 기회가 있어 내용을 추가한다.(확실한 내용은 아님)

샤딩은 데이터가 여러 스토리지에 분리된 것을 말하고, 파티셔닝은 한 스토리지 내에서 물리적으로 분리된 것을 말한다.
...
# NoSQL
NoSQL은 관계형 데이터 베이스가 아닌 모든 데이터 베이스를 일컫는 단어다.
